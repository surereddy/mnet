...
// Get attempts to retrieve giving item from the store.
func (p PersistenceStoreMock) Get(name string) ([]byte, error) {
	if p.GetFunc == nil {
...
// Save attempts adding giving data into store keyed by provided name.
func (p PersistenceStoreMock) Save(name string, data []byte) error {
